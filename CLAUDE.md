...
# CLAUDE.md — SuperCompany Coding 开发执行规范

本文档用于指导 Claude Code / Minimax M2.7 / DeepSeek / 其他 AI Coding Agent 开发本项目。

## 0. 当前仓库状态

**这是纯文档/规范仓库，尚无任何代码。** 所有 `.md` 文件为 SuperCompany Coding 桌面 App 的产品需求、架构设计和开发规范。代码实现将从零开始，严格按本文档和 `TASKS.md` 的 Phase 顺序推进。

`AGENTS.md` 与 `CLAUDE.md` 内容几乎一致，仅针对不同 AI 工具的品牌名称不同。修改本文件时需同步更新 `AGENTS.md`（将 "Claude Code" 替换为 "Codex"）。
...
pnpm typecheck
pnpm test
pnpm test -- --filter <test-file>  # 运行单个测试
...
### 12.2 必须实现 Orchestrator Runtime
...
### 12.3 默认主界面是 Project Command Center
