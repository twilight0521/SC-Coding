    ...
    if path.join("package.json").exists() {
        ...
        if path.join("package.json").exists() {
            if let Ok(content) = std::fs::read_to_string(path.join("package.json")) {
                if content.contains("\"test\"") || content.contains("\"vitest\"") || content.contains("\"jest\"") {
...
fn build_test_command(framework: &str, project_path: &str, test_filter: Option<&str>) -> (String, String) {
...
pub fn run_tests(
    ...
    let (command, working_dir) = build_test_command(&framework, &project_path, test_filter.as_deref());
...
pub fn quick_test(
    ...
    Ok(TestResult {
        ...
        duration_ms,
        output: combined_output,
        error: if passed { None } else { Some("Quick test failed".to_string()) },
        passed,
    })
}
